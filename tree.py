...
class TreePrinter():
    ...
    def _getStartSVG(self):
        ...
        for i in range(self.last_level):
            ...
            self.svg_representation += f'<line x1="{0}" y1="{y}" x2="{self.width}" y2="{y}" style="stroke:#898989;stroke-width:2" />\n'

        # vertical line
        self.svg_representation += f'<line x1="{self.sizeDataBox}" y1="{0}" x2="{self.sizeDataBox}" y2="{self.height}" style="stroke:#898989;stroke-width:2" />\n'
    ...
    def _addMetrics(self):
        for i, val in enumerate(self.treeHistory):
            ...
            r1 = f'<text dominant-baseline = "central" font-size="{self.node_font_size}" style = "fill: #000000; " >'
